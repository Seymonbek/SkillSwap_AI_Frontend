...
import { chatService, authService } from '@/shared/api';
...
export const VideoPage = () => {
  ...
  const startCall = async (id) => {
    try {
      ...
      // Check Barter Session Limit First
      if (sessionId) {
        const res = await startBarterSession(sessionId);
        if (!res.success) {
          setLimitError(true);
  ...
  const endCall = async () => {
    ...
    if (sessionId) {
      try {
        await completeBarterSession(sessionId);
  ...
  const handleSubmitReview = async (e) => {
    ...
    try {
      const receiverId = activeCall?.participant_id || activeCall?.participant?.id;
      if (receiverId) {
      ...
      {showReviewModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="glass-card w-full max-w-md p-6">
            ...
            <p className="text-slate-400 mb-6">Ushbu dars uchun tokenni o&apos;tkazish muvaffaqiyatli bajarildi. Iltimos, mentorga baho bering.</p>
            <form onSubmit={handleSubmitReview} className="space-y-4">
              ...
              <button type="submit" disabled={submittingReview} className="btn-primary w-full py-3 mt-2">
                {submittingReview ? <Loader2 className="w-5 h-5 animate-spin mx-auto" /> : 'Baholash va Chiqish'}
              </button>
            </form>
          </div>
        </div>
